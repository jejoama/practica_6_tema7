...
#usar factor
#cuando hago factor, guardo un numero por lo que cuando haga operaciones sera con el numero y no por su nombre
library(tidyverse)
datos <- read_tsv("data/NosAutoanalizamos_TD2019 - Hoja 1 - Clean.tsv") %>%
  mutate_at(vars(ends_with(".cat")), as.factor)
datos
#ends_with(".cat") selecciona todas las columnas cuyo nombre termina en .cat.
...
pivot_longer(datos, cols=ends_with(".num"), names_to = "asignatura", values_to = "valor") %>% group_by(asignatura) %>% summarise(mean=mean(valor, na.rm=T), sd = sd(valor, na.rm=T), iqr = IQR(valor, na.rm=T), median = median(valor, na.rm=T))
...
pivot_longer(datos, cols=ends_with(".num"), names_to = "asignatura", values_to = "valor") %>% filter(asignatura=="ALG.num") %>% ggplot(aes(valor)) + geom_histogram(aes(y=stat(density))) + geom_density() + geom_vline(aes(xintercept = mean(valor, na.rm = T)), color="red", size=1)
```
...
library(car)
library(tidyverse)
valores <-  datos %>% pivot_longer(cols=ends_with(".num"), names_to = "asignatura", values_to = "valor") %>% filter(asignatura=="AMM.num", !is.na(valor), is.finite(valor))
...
test_alg_sex <- chisq.test(tabla_alg_sex, correct = FALSE)
