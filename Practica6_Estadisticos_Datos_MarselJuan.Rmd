...
pivot_longer(datos, cols=ends_with(".num"), names_to = "asignatura", values_to = "valor") %>% filter(asignatura=="ALG.num") %>% ggplot(aes(valor)) + geom_histogram(aes(y=stat(density))) + geom_density() + geom_vline(aes(xintercept = mean(valor)), color="red", size=15)
```
...
library(car)
valores <-  datos %>% pivot_longer(cols=ends_with(".num"), names_to = "asignatura", values_to = "valor") %>% filter(asignatura=="AMM.num")
